...
L'étude démontre que la viabilité de "la moulinette" repose sur un équilibre strict entre sa capacité d'admission et sa fluidité de sortie. Si l'augmentation du nombre de serveurs ($K \ge 4$) est indispensable pour réduire le taux de refus à l'entrée, elle devient contre-productive si le canal de sortie ($\mu_f$) n'est pas dimensionné proportionnellement pour éviter l'explosion des "pages blanches". Pour garantir un système à la fois performant et équitable, la politique FIFO couplée à un backup systématique s'impose comme la solution la plus robuste, offrant une latence stable sans sacrifier de population spécifique. En effet, nos simulations démontrent que les solutions testées, telles que le \textit{Split} ou la priorisation, ne parviennent qu'à déplacer la saturation sans la résoudre : le gain de performance obtenu pour une population se fait systématiquement au prix d'une dégradation inacceptable pour l'autre. 
